...
describe("End to end test", () => {
    ...
    it("set trigger should fail if chat is not initialized", () => {
        chat.fireSet("chat1", "user1", "trigger1 m 15 17:30 M+1,t1,s0,m0 M+2,t1,s0,m0");
        expect(chat.send).toHaveBeenCalledTimes(1);
        expect(chat.send).toHaveBeenCalledWith("chat1", "Fehler: Error: CHAT_NOT_EXISTING: {}");
    });
    it("chat should be initialized", () => {
        chat.fireInitChat("chat1", "user1");
        expect(chat.send).toHaveBeenCalledTimes(1);
        expect(chat.send).toHaveBeenCalledWith("chat1", "Initialisierung des Chats erfolgreich.");
    });
    it("set trigger should succeed if chat is initialized", () => {
        chat.fireSet("chat1", "user1", "trigger1 m 15 17:30 M+1,t1,s0,m0 M+2,t1,s0,m0");
        expect(chat.send).toHaveBeenCalledTimes(1);
        expect(chat.send).toHaveBeenCalledWith("chat1", "Setzen von trigger1 erfolgreich.");
    });
    it("should send the events if a trigger fires", () => {
        timer.fireTrigger("chat1|trigger1");
        expect(chat.send).toHaveBeenCalledTimes(1);
